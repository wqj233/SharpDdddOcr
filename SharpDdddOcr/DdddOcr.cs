...
namespace SharpDdddOcr;

public class DdddOcr : IDisposable
{
    private readonly InferenceSession _session;
    private readonly List<string> _charset = new List<string> { "", "掀", "袜", "顧", "徕", "榱", "荪", "浡", "其", "炎", "玉", "恩", "劣", "徽", "廉", "桂", "拂", "鳊", "撤",
        ...
        "設", "⒆" };
    // TODO: OnnxRuntime的GPU包过大，请自行适配
    public DdddOcr(/*bool useGpu = false,int deviceId = 0*/)
    {
        var graphPath = Path.Combine(Directory.GetCurrentDirectory(), "common.onnx");
        ...
        // 创建实例
        _session = new InferenceSession(graphPath, options);
    }
    public string Classification(byte[] img)
    {
        // 加载图片
        using var image = Image.Load<Rgba32>(img);
        // 计算调整后的宽度
        var newWidth = (int)(image.Width * (64.0 / image.Height));
        image.Mutate(x => x
            .Resize(new ResizeOptions
            {
                Size = new Size(newWidth, 64),
        ...
        // 创建并填充张量
        var tensor = new DenseTensor<float>(new[] { 1, 1, 64, newWidth });
        for (var y = 0; y < 64; y++)
        {
        ...
        foreach (var item in output)
        {
            ...
            lastItem = item;
            if (item != 0 && item < _charset.Count)
                result.Add(_charset[(int)item]);
        }
